...
Player flags control what **non-members** can do inside your region. Members and owners bypass these restrictions, so the table below is about protecting your land from strangers and visitors. Separate per-member flags let owners fine-tune what each trusted player may do.

The defaults shown here match `flags.yml`. Flags marked **Allow** (such as `passthrough`, `pickup-items`, and `elytra`) keep the region open to foot traffic by default, while anything that could disturb builds or inventories defaults to **Deny**. Each region can override these values independently.
...
| `punch-sulfur-cubes`      | 68719476736  |     —     | :material-close: **Deprecated**      |
| `throw-eggs`              | 137438953472 |   Deny    | Eggs hatch inside region             |

!!! info "Deprecated Flags"

    `punch-sulfur-cubes` is deprecated. It remains in the list only for compatibility with older configurations.

## Changing Player Flags

- **Global (non-members)**: `/hs flags global [flag] [allow/deny]`
- **Per member**: `/hs flags member [player] [flag] [allow/deny]`
- **Defaults for new regions**: edit `default-players-flags` in `flags.yml`

See [Flags Overview](Flags%20Overview.md) for the full workflow, and [Disabled Flags](Disabled%20Flags.md) to lock a flag so nobody can change it.
