...
public final class Resources {
	...
	private static File resolveLocaleFile(File dir, String setting, String fallbackName) {
		...
		if (!file.exists()) {
			Logger.info("Locale file '" + name + "' not found – falling back to " + fallbackName);
	...
	private static void ensureFile(Homestead plugin, File file, String resourceName) throws IOException {
		...
		if (stream == null) {
			Logger.info("Bundled resource '" + resourceName + "' not found in JAR.");
			return;
		}
		FileUtils.copyInputStreamToFile(stream, file);
	}
	...
	private static void validateAndFix(
		...
		if (!validator.validate()) {
			Logger.info("'" + targetFile.getName() + "' has missing keys – auto-fixing from defaults...");
			validator.fix();
