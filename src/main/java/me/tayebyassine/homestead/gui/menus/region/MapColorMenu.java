...
/**
 * Paginated map color picker for regions with dynamic map support.
 *
 * <p>Displays available color options as stained glass panes with hex codes. Selected color
 * is highlighted with an enchant glint.</p>
 */
public final class MapColorMenu {
    private static final String MENU_KEY = "map_color";
    ...
    public MapColorMenu(Player player, Region region) {
        ...
        PaginationMenu.builder(MENU_KEY, new Placeholder().add("{region}", region.getName()), 9 * 5)
                .nextPageItem(MenuButtons.getNextPageButton())
                .prevPageItem(MenuButtons.getPreviousPageButton())
                .items(getItems(player, region))
                .fillEmptySlots()
                .goBack((_player, event) -> new MiscellaneousSettingsMenu(player, region))
    ...
    private void handleMapColorClick(Player player, Region region, PaginationMenu.ClickContext context) {
        if (context.index() >= colorEntries.size()) return;
        ...
        MapColorEntry entry = colorEntries.get(context.index());
    ...
    private record MapColorEntry(String colorName, int colorValue, Material glassPaneMaterial, String displayName,
                                 String hexCode) {
    }
}