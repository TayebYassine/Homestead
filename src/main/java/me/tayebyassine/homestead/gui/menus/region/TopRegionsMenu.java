...
/**
 * Paginated top regions leaderboard with sorting and public/private toggle.
 *
 * <p>Sort by bank, chunks, members, creation date, or rating. Operators can toggle between
 * public-only and all regions. Shows the player's selected target region rank.</p>
 */
public final class TopRegionsMenu {
    private static final String MENU_KEY = "top_regions";
    ...
    public TopRegionsMenu(Player player, boolean isPublic, SortMode sortMode) {
        ...
        PaginationMenu.Builder builder = PaginationMenu.builder(MENU_KEY, 9 * 5)
                ...
                .actionButton(0, MenuButtons.getButton(MENU_KEY, sortMode.buttonId), handleCycleSort(player, sortMode))
                .actionButton(2, MenuButtons.getButton(MENU_KEY, 6, new Placeholder()
        ...
        if (selected != null) {
            builder.actionButton(1, MenuButtons.getButton(MENU_KEY, 7, new Placeholder()
    ...
    private List<ItemStack> getItems(Player player) {
        ...
        for (int i = 0; i < regions.size(); i++) {
            ...
            items.add(MenuButtons.getButton(MENU_KEY, 0, placeholder));
        }
    ...
    public enum SortMode {
        BANK(1, RegionManager.RegionSorting.BANK),
        CHUNKS(2, RegionManager.RegionSorting.CHUNKS_COUNT),
        MEMBERS(3, RegionManager.RegionSorting.MEMBERS_COUNT),
        OLDEST(4, RegionManager.RegionSorting.CREATION_DATE),
        RATING(5, RegionManager.RegionSorting.RATING);

        final int buttonId;
        final RegionManager.RegionSorting sorting;

        SortMode(int buttonId, RegionManager.RegionSorting sorting) {
