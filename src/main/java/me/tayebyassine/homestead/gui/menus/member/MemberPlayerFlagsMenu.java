...
/**
 * Per-member player flag configuration menu.
 *
 * <p>Allows toggling individual player flags for a specific region member. Includes a bulk
 * enable/disable option at index 0 (left-click enable all, right-click disable all).</p>
 */
public final class MemberPlayerFlagsMenu {
    private static final String MENU_KEY = "region_member_flags";
    private static final int BULK_INDEX = 0;

    public MemberPlayerFlagsMenu(Player player, Region region, RegionMember member) {
    ...
    private void handleBulkToggle(Player player, Region region, RegionMember member, PaginationMenu.ClickContext context) {
        boolean enableAll = context.event().isLeftClick();
        boolean disableAll = context.event().isRightClick();
        ...
        context.instance().setItems(buildItemsList(member));
    ...
    private void handleSingleToggle(Player player, Region region, RegionMember member, PaginationMenu.ClickContext context, int index) {
        ...
        PlayerSound.play(player, PlayerSound.PredefinedSound.CLICK);
        context.instance().replaceSlot(index, MenuButtons.getFlagButton(MENU_KEY, 1, flagString, !isSet));
    }

    private List<ItemStack> buildItemsList(RegionMember member) {
        List<ItemStack> items = new ArrayList<>();
        items.add(MenuButtons.getButton(MENU_KEY, 0));

        for (String flagString : PlayerFlag.getFlags()) {
            boolean value = FlagCalculator.isFlagSet(member.getPlayerFlags(), PlayerFlag.parse(flagString));
            items.add(MenuButtons.getFlagButton(MENU_KEY, 1, flagString, value));
        }
