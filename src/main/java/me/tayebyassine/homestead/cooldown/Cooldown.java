package me.tayebyassine.homestead.cooldown;
...
public final class Cooldown {
    ...
    public static boolean hasCooldown(Player player) {
        ...
        if (COOLDOWNS.containsKey(id)) {
            ...
            return !(data.type().ignoreOperators() && PlayerUtility.isOperator(player));
        }
    ...
    public static boolean hasCooldown(Player player, Type type) {
        ...
        if (COOLDOWNS.containsKey(id)) {
            ...
            int cooldown = type.getCooldown();
            if (data.type() == type && cooldown <= 1) return false;

            return data.type() == type && !(type.ignoreOperators() && PlayerUtility.isOperator(player));
        }
    ...
    public enum Type {
        ...
        public boolean ignoreOperators() {
            return Resources.<RegionsFile>get(ResourceType.Regions).isCooldownIgnoreOperators(key);
        }

        public int getCooldown() {
            return Resources.<RegionsFile>get(ResourceType.Regions).getCooldownValue(key);
        }
    }

    private record CooldownData(Type type, long endTime) {
    }
}