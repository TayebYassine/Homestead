package me.tayebyassine.homestead.util.java;

import me.tayebyassine.homestead.managers.MemberManager;
import me.tayebyassine.homestead.managers.RegionManager;
import me.tayebyassine.homestead.models.Region;
import me.tayebyassine.homestead.models.RegionMember;
import me.tayebyassine.homestead.models.War;
import me.tayebyassine.homestead.resources.ResourceType;
import me.tayebyassine.homestead.resources.Resources;
import me.tayebyassine.homestead.resources.files.LanguageFile;
import org.bukkit.*;

import java.text.SimpleDateFormat;
import java.util.List;
import java.util.Map;
import java.util.stream.Collectors;

public final class Formatter {
    /**
     * Apply placeholders to a string.
     *
     * @param string      The string
     * @param placeholder The placeholders
     */
    public static String applyPlaceholders(String string, Placeholder placeholder) {
        if (placeholder == null) placeholder = new Placeholder();

        Map<String, String> replacements = placeholder.build();

        return applyPlaceholders(string, replacements);
    }

    private static String applyPlaceholders(String string, Map<String, String> replacements) {
        if (string == null) {
            return "NULL";
        }

        for (Map.Entry<String, String> entry : replacements.entrySet()) {
            string = string.replace(entry.getKey(), entry.getValue() == null ? "NULL" : entry.getValue());
        }

        return string;
    }

    /**
     * Get beautiful and readable {@link Location} string.
     *
     * @param location The location
     */
    public static String getLocation(Location location) {
        World world = location.getWorld();

        return applyPlaceholders(
                Resources.<LanguageFile>get(ResourceType.Language).getFormatterLocation(),
                new Placeholder()
                        .add("{world}", world == null ? "UnknownWorld" : world.getName())
                        .add("{x}", NumberUtils.truncate(location.getX()))
                        .add("{y}", NumberUtils.truncate(location.getY()))
                        .add("{z}", NumberUtils.truncate(location.getZ()))
        );
    }

    /**
     * Get beautiful and readable {@link Chunk} string.
     *
     * @param chunk The chunk
     */
    public static String getLocationChunk(Chunk chunk) {
        World world = chunk.getWorld();

        return applyPlaceholders(
                Resources.<LanguageFile>get(ResourceType.Language).getFormatterChunk(),
                new Placeholder()
                        .add("{world}", world.getName())
                        .add("{x}", NumberUtils.truncate(chunk.getX()))
                        .add("{z}", NumberUtils.truncate(chunk.getZ()))
        );
    }

    /**
     * Get beautiful and readable balance string.
     *
     * @param amount The amount
     */
    public static String getBalance(double amount) {
        String balance = NumberUtils.convertToBalance(amount);
        String format = Resources.<LanguageFile>get(ResourceType.Language).getFormatterBalance();

        return format.replace("{balance}", balance);
    }

    /**
     * Get beautiful and readable date timestamp string.
     *
     * @param date The date timestamp
     */
    public static String getDate(long date) {
        String pattern = Resources.<LanguageFile>get(ResourceType.Language).getFormatterDateFormat();
        SimpleDateFormat simpleDateFormat = new SimpleDateFormat(pattern);

        String formatted = simpleDateFormat.format(date);
        String dateWithAgo = Resources.<LanguageFile>get(ResourceType.Language).getFormatterDate();

        Placeholder placeholder = new Placeholder()
                .add("{date}", formatted)
                .add("{time-ago}", getAgo(date));

        return applyPlaceholders(dateWithAgo, placeholder);
    }

    /**
     * Get beautiful and readable rate string.
     *
     * @param rate The average rating
     */
    public static String getRating(double rate) {
        return getRating(Math.round((float) rate));
    }

    /**
     * Get beautiful and readable rate string.
     *
     * @param rate The rate, from 1 to 5.
     */
    public static String getRating(int rate) {
        if (rate == 0 || rate > 5) {
            return getNone();
        }

        String star = Resources.<LanguageFile>get(ResourceType.Language).getVariableStar();

        return getStarColor(rate) + star.repeat(rate);
    }

    private static String getStarColor(int rate) {
        return Resources.<LanguageFile>get(ResourceType.Language).getVariableStarColor(rate);
    }

    /**
     * Get beautiful and readable remaining time string.
     *
     * @param time The remaining time
     */
    public static String getRemainingTime(long time) {
        long currentTime = System.currentTimeMillis();

        long differenceMillis = time - currentTime;
        long totalSeconds = differenceMillis / 1000;

        long days = totalSeconds / 86400;
        long remaining = totalSeconds % 86400;

        long hours = remaining / 3600;
        remaining = remaining % 3600;

        long minutes = remaining / 60;
        long seconds = remaining % 60;

        return applyPlaceholders(
                Resources.<LanguageFile>get(ResourceType.Language).getFormatterDuration(),
                new Placeholder()
                        .add("{d}", days)
                        .add("{h}", hours)
                        .add("{m}", minutes)
                        .add("{s}", seconds)
        );
    }

    /**
     * Get beautiful and readable duration string from milliseconds.
     *
     * @param millis The duration in milliseconds
     */
    public static String getDuration(long millis) {
        if (millis <= 0) {
            return getPermanent();
        }

        long totalSeconds = millis / 1000;

        long days = totalSeconds / 86400;
        long remaining = totalSeconds % 86400;

        long hours = remaining / 3600;
        remaining = remaining % 3600;

        long minutes = remaining / 60;
        long seconds = remaining % 60;

        return applyPlaceholders(
                Resources.<LanguageFile>get(ResourceType.Language).getFormatterDuration(),
                new Placeholder()
                        .add("{d}", days)
                        .add("{h}", hours)
                        .add("{m}", minutes)
                        .add("{s}", seconds)
        );
    }

    /**
     * Get beautiful and readable {@link OfflinePlayer}'s playtime string.
     *
     * @param player The player
     */
    public static String getPlayerPlaytime(OfflinePlayer player) {
        long totalMinutes = getPlayerMinutes(player);
        long totalSeconds = totalMinutes * 60;

        long days = totalSeconds / 86400;
        long remainingSeconds = totalSeconds % 86400;

        long hours = remainingSeconds / 3600;
        remainingSeconds %= 3600;

        long minutes = remainingSeconds / 60;
        long seconds = remainingSeconds % 60;

        return applyPlaceholders(
                Resources.<LanguageFile>get(ResourceType.Language).getFormatterDuration(),
                new Placeholder()
                        .add("{d}", days)
                        .add("{h}", hours)
                        .add("{m}", minutes)
                        .add("{s}", seconds)
        );
    }

    private static long getPlayerMinutes(OfflinePlayer player) {
        return player.getStatistic(Statistic.PLAY_ONE_MINUTE) / (20L * 60L);
    }

    public static String formatPaginationMenuTitle(String title, int currentPage, int totalPages) {
        return applyPlaceholders(
                Resources.<LanguageFile>get(ResourceType.Language).getFormatterGuiPaginationTitle(),
                new Placeholder()
                        .add("{title}", title)
                        .add("{current-page}", currentPage)
                        .add("{total-pages}", totalPages)
        );
    }

    public static String formatPrivateChat(String regionName, String sender, String message) {
        return applyPlaceholders(
                Resources.<LanguageFile>get(ResourceType.Language).getFormatterPrivateChat(),
                new Placeholder()
                        .add("{region}", regionName)
                        .add("{sender}", sender)
                        .add("{message}", message)
        );
    }

    /**
     * Get beautiful and readable list of regions that a player owns.
     *
     * @param player The player
     */
    public static String getPlayerOwnedRegions(OfflinePlayer player) {
        List<Region> regions = RegionManager.getRegionsOwnedByPlayer(player);

        if (regions.isEmpty()) {
            return getNone();
        }

        String format = Resources.<LanguageFile>get(ResourceType.Language).getFormatterPlayerRegions();

        return regions.stream()
                .map((region) -> format.replace("{region}", region.getName()))
                .collect(Collectors.joining(Resources.<LanguageFile>get(ResourceType.Language).getFormatterPlayerRegionsJoining()));
    }

    /**
     * Get beautiful and readable list of region that a player is trusted in.
     *
     * @param player The player
     */
    public static String getPlayerTrustedRegions(OfflinePlayer player) {
        List<Region> regions = RegionManager.getRegionsHasPlayerAsMember(player);

        if (regions.isEmpty()) {
            return getNone();
        }

        String format = Resources.<LanguageFile>get(ResourceType.Language).getFormatterPlayerRegions();

        return regions.stream()
                .map((region) -> format.replace("{region}", region.getName()))
                .collect(Collectors.joining(Resources.<LanguageFile>get(ResourceType.Language).getFormatterPlayerRegionsJoining()));
    }

    /**
     * Get beautiful and readable list of members of a region.
     *
     * @param region The region
     */
    public static String getMembersOfRegion(Region region) {
        List<RegionMember> members = MemberManager.getMembersOfRegion(region);

        if (members.isEmpty()) {
            return getNone();
        }

        String format = Resources.<LanguageFile>get(ResourceType.Language).getFormatterRegionMembers();

        return members.stream()
                .map((member) -> format.replace("{playername}",
                                member.getPlayerName()
                        )
                )
                .collect(Collectors.joining(Resources.<LanguageFile>get(ResourceType.Language).getFormatterRegionMembersJoining()));
    }

    /**
     * Get beautiful and readable list of regions part of a war.
     *
     * @param war The war
     */
    public static String getRegionsOfWar(War war) {
        List<Region> regions = war.getRegions();

        if (regions.isEmpty()) {
            return getNone();
        }

        String format = Resources.<LanguageFile>get(ResourceType.Language).getFormatterWarRegions();

        return regions.stream()
                .map((region) -> format.replace("{region}", region.getName()))
                .collect(Collectors.joining(Resources.<LanguageFile>get(ResourceType.Language).getFormatterWarRegionsJoining()));
    }

    public static String getNone() {
        return Resources.<LanguageFile>get(ResourceType.Language).getVariableNone();
    }

    public static String getNA() {
        return Resources.<LanguageFile>get(ResourceType.Language).getVariableNa();
    }

    public static String getPermanent() {
        return Resources.<LanguageFile>get(ResourceType.Language).getVariablePermanent();
    }

    public static String getNever() {
        return Resources.<LanguageFile>get(ResourceType.Language).getVariableNever();
    }

    public static String getBoolean(boolean value) {
        return value ? Resources.<LanguageFile>get(ResourceType.Language).getVariableIsTrue() : Resources.<LanguageFile>get(ResourceType.Language).getVariableIsFalse();
    }

    public static String getToggle(boolean value) {
        return value ? Resources.<LanguageFile>get(ResourceType.Language).getVariableIsEnabled() : Resources.<LanguageFile>get(ResourceType.Language).getVariableIsDisabled();
    }

    public static String getFlagState(boolean value) {
        return value ? Resources.<LanguageFile>get(ResourceType.Language).getVariableFlagSet() : Resources.<LanguageFile>get(ResourceType.Language).getVariableFlagUnset();
    }

    /**
     * Get the player status; <b>Online</b>, <b>Offline</b>, or server <b>Banned</b>.
     */
    public static String getPlayerStatus(OfflinePlayer player) {
        return Bukkit.getBannedPlayers().contains(player) ? Resources.<LanguageFile>get(ResourceType.Language).getVariableBanned()
                : (player.isOnline() ? Resources.<LanguageFile>get(ResourceType.Language).getVariableOnline()
                : Resources.<LanguageFile>get(ResourceType.Language).getVariableOffline());
    }

    /**
     * Get beautiful and readable duration "ago" string.
     *
     * @param time The date timestamp
     */
    public static String getAgo(long time) {
        long currentTime = System.currentTimeMillis();

        long differenceMillis = currentTime - time;
        long totalSeconds = differenceMillis / 1000;

        long days = totalSeconds / 86400;
        long remaining = totalSeconds % 86400;

        long hours = remaining / 3600;
        remaining = remaining % 3600;

        long minutes = remaining / 60;
        long seconds = remaining % 60;

        if (days != 0) {
            return Resources.<LanguageFile>get(ResourceType.Language).getFormatterAgoDays().replace("{v}", String.valueOf(Math.abs(days)));
        }

        if (hours != 0) {
            return Resources.<LanguageFile>get(ResourceType.Language).getFormatterAgoHours().replace("{v}", String.valueOf(Math.abs(hours)));
        }

        if (minutes != 0) {
            return Resources.<LanguageFile>get(ResourceType.Language).getFormatterAgoMinutes().replace("{v}", String.valueOf(Math.abs(minutes)));
        }

        return Resources.<LanguageFile>get(ResourceType.Language).getFormatterAgoSeconds().replace("{v}", String.valueOf(Math.abs(seconds)));
    }
}
