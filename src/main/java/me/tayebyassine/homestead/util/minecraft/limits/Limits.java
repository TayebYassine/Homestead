package me.tayebyassine.homestead.util.minecraft.limits;
...
public final class Limits {
    ...
    private static int getBaseLimitValue(OfflinePlayer player, LimitType limit) {
        ...
        Object playerOverride = Resources.<LimitsFile>get(ResourceType.Limits).getPlayerLimit(player.getName(), limitKey);
        ...
        switch (method) {
            case STATIC -> {
                boolean isOp = PlayerUtility.isOperator(player);

                Object staticValue = Resources.<LimitsFile>get(ResourceType.Limits).getStaticLimit(isOp, limitKey);
            ...
            case GROUPS -> {
                ...
                Object groupValue = Resources.<LimitsFile>get(ResourceType.Limits).getGroupLimit(group, limitKey);
    ...
    private static int getPermissionBasedLimit(OfflinePlayer player, LimitType limit) {
        ...
        List<String> priorityGroups = Resources.<LimitsFile>get(ResourceType.Limits)
                .getPermissionsPriority();

        if (priorityGroups.isEmpty()) {
            priorityGroups = Resources.<LimitsFile>get(ResourceType.Limits)
        ...
        for (String group : priorityGroups) {
            if (effectivePerms.contains("homestead.group." + group)) {
                Object limitValue = Resources.<LimitsFile>get(ResourceType.Limits).getPermissionLimit(group, limitKey);
                if (limitValue != null) {
    ...
    public static LimitMethod getLimitsMethod() {
        String method = Resources.<LimitsFile>get(ResourceType.Limits).getLimitsMethod();
